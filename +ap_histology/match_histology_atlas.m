...
histology_toolbar_guidata = guidata(histology_toolbar_gui); % Load GUI data
atlasPaths = histology_toolbar_guidata.atlasPaths; % Get atlasPaths
atlasType = histology_toolbar_guidata.atlasType;
gui_data.atlasType = histology_toolbar_guidata.atlasType; %add to gui_data
% Load atlas

disp(['Loading ', atlasType, ' atlas...']);

gui_data.tv = readNPY(which(atlasPaths.template)); % Use 'which' to locate the file
gui_data.av = readNPY(which(atlasPaths.annotation));
gui_data.st = ap_histology.loadStructureTree(which(atlasPaths.structure_tree));
% disp('Loaded successfully.');

% Get images (from path in GUI)
% Retrieve atlas paths from gui_data
histology_toolbar_guidata = guidata(histology_toolbar_gui); % Load GUI data
...
gui_width_fraction = 0.8; % fraction of screen width to occupy
...
% Remove any whitespace from each string
ccf_color_hex = cellfun(@strtrim, ccf_color_hex, 'UniformOutput', false);
% For any string that is not 6 characters long, pad it on the left with zeros.
ccf_color_hex = cellfun(@(x) pad(x,6,'left','0'), ccf_color_hex, 'UniformOutput', false);
...
switch user_confirm
    case 'Yes'
        ...
        if strcmp(gui_data.atlasType, 'adult')
            ...
            save_fn = fullfile(gui_data.save_path, ['histology_', gui_data.atlasType, 'ccf.mat']);
        end
