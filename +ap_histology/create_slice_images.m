...
for curr_channel = 1:n_channels
    ...
    im_montage{curr_channel} = curr_montage.CData;
      ...
      if any(im_montage{curr_channel}(:) > 0)
        ...
        % If there are no values greater than zero, skip this step
        disp('Warning: Skipping channel due to zero values.');
        % Choose a default color
        channel_color{curr_channel} = [0 0 1]; % Default to blue
        cmin = 0;
        cmax = 1; % Set default caxis limits if no values are present
      end

    channel_caxis(curr_channel,:) = [cmin,cmax];
end
% 
% cmin_all = min(channel_caxis(:,1));
% cmax_all = max(channel_caxis(:,2));
% caxis([cmin_all, cmax_all]);
