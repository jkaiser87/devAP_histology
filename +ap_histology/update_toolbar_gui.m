...
% Use dir to list files matching the pattern
...
matching_files = dir(fullfile(gui_data.save_path, 'histology_*ccf.mat'));
atlas_slices_present = ~isempty(gui_data.save_path) && ~isempty(matching_files);
matching_files = dir(fullfile(gui_data.save_path, 'atlas2histology_*tform.mat'));
atlas_alignment_present = ~isempty(gui_data.save_path) && ~isempty(matching_files);
...
atlas_text = sprintf('Atlas: %s', gui_data.atlasType);
